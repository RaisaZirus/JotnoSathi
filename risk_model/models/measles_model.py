...
Rebalanced v2.1 — outbreak weight reduced to prevent domination.
"""
import os
import pandas as pd
...
class MeaslesModel(BaseRiskModel):
    ...
    weights = {
        "measles_outbreak_weight": 0.35,  # was 0.40 — reduced
        "vaccination_gap":         0.35,  # was 0.30 — increased (more meaningful)
        "u5_mortality_rate":       0.20,  # was 0.15
        "unmet_health_need_pct":   0.10,  # was 0.15 — reduced
    }
    ...
    # 2026 outbreak distribution — rebalanced to reflect realistic spread
    # Mymensingh and Dhaka are epicentres but shouldn't both be at 2.0
    OUTBREAK_WEIGHTS = {
        "Mymensingh": 2.0,   # confirmed epicentre
        "Dhaka":      1.6,   # was 2.0 — reduced
        "Barishal":   1.4,   # was 1.6
        "Chattogram": 1.0,   # was 1.2
        "Sylhet":     0.8,   # was 1.0
        "Rajshahi":   0.7,   # was 0.9
        "Rangpur":    0.6,   # was 0.7
        "Khulna":     0.5,   # was 0.8
    }
    ...
    def build_features(self, data_dir: str) -> pd.DataFrame:
        ...
        try:
            ...
            u5mr = (df[df["Indicator"] == "Under-five mortality rate"]
            ...
            defaults = {"Sylhet": 55, "Mymensingh": 50, "Barishal": 48,
